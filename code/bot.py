...
from data import user_load, user_save, locations_load, location_save, items_2
from game import up_time, check_tired_state
import time
...
def sleep_answer(message):
    ...
    try:
        ...
        if (hrs == 10 and mins == 0) or (0 <= hrs < 10 and 0 < mins < 60):
            ...
            bot.send_message(message.chat.id, text='сколько в часе минут? подсказка: 1 - 59')
        else:
            bot.send_message(message.chat.id, text='вы можете ввести часы только в диапазоне от 0 до 10')
    except ValueError:
...
def scouting(call) -> None:
    ...
    if len(locations[location]['loot']) == 1:
        ...
        txt_3 = found_items_list[0]
        bot.send_message(m_id, text=txt_3, reply_markup=loot_markup)
...
def scouting_2(call):
    ...
    button_2_2 = InlineKeyboardButton('оcтавить', callback_data=f'{x}_drop')
    loot_markup.add(button_2_1, button_2_2)
    pick_or_drop(m_id, choice, x)



def pick_or_drop(m_id, choice, x):
...
#опус магнум или сумасбродное безумие?